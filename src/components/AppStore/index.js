...
class AppStore extends Component {
  state = {activTab: tabsList[0].tabId, searchInput: ''}

  onCLickFunction = tabId => {
    this.setState({activTab: tabId})
  }

  getFilteredList = () => {
    const {activTab} = this.state
    const filteredList = appsList.filter(each => each.category === activTab)
    return filteredList
  }

  onChnageInputValue = event => {
    this.setState({searchInput: event.target.value})
  }

  render() {
    const {activTab, searchInput} = this.state
    const filteredList = this.getFilteredList(activTab)

    const searchFilteredList = filteredList.filter(eachApp =>
      ...
      <div className="bg-container">
        ...
        <div className="searc-box-container">
          <input
            type="search"
            placeHolder="Search"
            onChange={this.onChnageInputValue}
          />
          <button>
            <img
              alt="search icon"
        ...
        <ul className="ul-tab">
          {tabsList.map(each => (
            <TabItem
              ...
              isActive={each.tabId === activTab}
            />
          ))}
        </ul>
        <div className="semi">
          <ul className="ul-appItem">
            {searchFilteredList.map(each => (
